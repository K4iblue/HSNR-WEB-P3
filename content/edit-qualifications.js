...
document.querySelectorAll(".actions .delete").forEach((e) =>
  e.addEventListener("click", function (e) {
    ...
    const id = row.dataset.qualificationId;
    if (confirm("Soll der Eintrag wirklich gelöscht werden?")) {
      row.remove();
      fetch("/api/qualification/delete?index=" + id);
    }
  })
);
...
document.querySelectorAll(".actions .save").forEach((e) =>
  e.addEventListener("click", function (e) {
    ...
    if (window.editing == id) {
      ...
      qualification = {};
      qualification.id = id;
      qualification.title = title.value;
      qualification.desc = desc.value;

      title.dataset.initValue = title.value;
      desc.dataset.initValue = desc.value;

      fetch("/api/qualification/update", {
        method: "post",
...
document.querySelectorAll(".actions .add").forEach((e) =>
  e.addEventListener("click", function (e) {
    if (window.editing == -1) {
      ...
      fetch("/api/qualification/insert", {
        method: "post",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(qualification),
      });
